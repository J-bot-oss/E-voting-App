...
def get_detailed_statistics() -> dict:
    ...
    active_candidates = sum(
        1 for candidate in state.candidates.values() if candidate["is_active"]
    )

    total_voters = len(state.voters)
    verified_voters = sum(
        1 for voter in state.voters.values() if voter["is_verified"]
    )
    active_voters = sum(
        1 for voter in state.voters.values() if voter["is_active"]
    )

    total_stations = len(state.voting_stations)
    active_stations = sum(
        1 for station in state.voting_stations.values() if station["is_active"]
    )

    total_polls = len(state.polls)
    open_polls = sum(
        1 for poll in state.polls.values() if poll["status"] == "open"
    )
    closed_polls = sum(
        1 for poll in state.polls.values() if poll["status"] == "closed"
    )
    draft_polls = sum(
        1 for poll in state.polls.values() if poll["status"] == "draft"
    )
    ...
    for station_id, station in state.voting_stations.items():
        voter_count = sum(
            1 for voter in state.voters.values() if voter["station_id"] == station_id
        )
        load_percent = (
...
def filter_audit_log(filter_by: str, filter_term: str) -> list:
    ...
    if filter_by == "user":
        return [
            ...
            if filter_term.lower() in entry["user"].lower()
        ]
