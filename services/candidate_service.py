...
def create_candidate(form_data: dict):
    ...
    created_id = state.candidate_id_counter
    state.candidate_id_counter += 1
    save_data()

    return True, f"Candidate '{full_name}' created successfully! ID: {created_id}"
...
def search_candidates(search_by: str, search_term) -> list:
    """Search candidates using different criteria."""
